...
try:
    ...
    if "-----BEGIN PRIVATE KEY-----" not in creds_dict.get("private_key", ""):
        st.error("🚨 Error en private_key.")
        st.stop()
...
def generate_full_pdf(df, logo_path, deficit_data=None):
    ...
    for _, r in df.iterrows():
        ...
        pdf.cell(w[4], 6, clean_pdf_text(f"{safe_get(r, '%Distrib')}%"), 1)
        pdf.ln()

    # ARREGLO ERROR 1: Tabla Semanal Normalizada
    pdf.add_page(); pdf.set_font("Arial", 'B', 11); pdf.cell(0, 10, "2. Resumen Semanal", ln=True)
    try:
        # Copia para no dañar original
        df_calc = df.copy()
        # Normalizar nombres de columnas a minúsculas para buscar 'equipo' y 'pct'
        df_calc.columns = [c.lower().strip() for c in df_calc.columns]
        
        # Buscar columna de porcentaje (puede ser '%distrib', 'pct', etc)
        ...
        if col_pct and col_eq:
            ...
            weekly_stats.columns = ["Equipo", "Promedio"]
            ...
            for i, txt in enumerate(h_wk): pdf.cell(w_wk[i], 6, clean_pdf_text(txt), 1)
            pdf.ln(); pdf.set_font("Arial", '', 9)
            
            for _, row in weekly_stats.iterrows():
                pdf.set_x(start_x)
                pdf.cell(w_wk[0], 6, clean_pdf_text(str(row["Equipo"])[:50]), 1)
                pdf.cell(w_wk[1], 6, clean_pdf_text(f"{row['Promedio']:.1f}%"), 1)
                pdf.ln()
        else:
            pdf.cell(0, 6, "No se encontraron datos para el resumen semanal.", ln=True)
            
    except Exception as e:
        pdf.set_font("Arial", 'I', 9); pdf.cell(0, 6, clean_pdf_text(f"Error cálculo semanal: {str(e)}"), ln=True)

    # Déficit
    if deficit_data:
        pdf.add_page(); pdf.set_font("Arial", 'B', 12); pdf.cell(0, 10, "Reporte de Déficit", ln=True)
        pdf.set_font("Arial", 'B', 8)
        w2 = [20, 50, 20, 20, 70]; h2 = ["Piso", "Equipo", "Día", "Faltan", "Causa"]
        for i, txt in enumerate(h2): pdf.cell(w2[i], 6, clean_pdf_text(txt), 1)
        pdf.ln(); pdf.set_font("Arial", '', 8)
        for d in deficit_data:
            ...
            pdf.cell(w2[4], 6, clean_pdf_text(d.get('causa','')), 1)
            pdf.ln()
...
def confirm_delete_dialog(conn, u, f, a, p):
    st.warning(f"¿Anular {u} - {f}?"); c1,c2=st.columns(2)
...
if menu == "Vista pública":
    ...
    if df.empty: st.info("Sin datos.")
        ...
        with t2:
            ...
            if ds == "Todos":
                if st.button("Descargar PDF Semanal"):
                    m = create_merged_pdf(pi, conn, global_logo_path)
                    if m: st.download_button("Descargar", m, "semana.pdf", "application/pdf")
            else:
                subset = df[(df['piso']==pi) & (df['dia']==ds)]
                seats = dict(zip(subset['equipo'], subset['cupos']))
                conf = st.session_state.get('last_style_config', {})
                img_path = generate_colored_plan(pi, ds, seats, "PNG", conf, global_logo_path)
                
                p_n = pi.replace("Piso ","").strip()
                d_n = ds.lower().replace("é","e").replace("á","a")
                f_prev = COLORED_DIR / f"piso_{p_n}_{d_n}_combined.png"
                if f_prev.exists(): st.image(str(f_prev))
                else: st.warning("No generado aún.")
        ...
        if not df.empty:
            ...
            if dn != "Fin":
                ...
                if not rg.empty:
                    ...
                    occ = len(list_reservations_df(conn)[lambda x: (x["reservation_date"].astype(str)==str(fe)) & (x["piso"]==pi)])
                    disp = tot - occ
                    st.metric("Cupos Disponibles", f"{disp}/{tot}")
                ...
                else: st.warning("Sin configuración de cupos libres.")
            else: st.error("Fin de semana cerrado.")

    # ARREGLO ERROR 3: Lista de Salas y Lógica
    elif op == "Sala":
        # Nuevas opciones de sala
        salas_disponibles = [
        ...
        sl = st.selectbox("Sala", salas_disponibles)
        
        # Lógica automática para detectar piso
        piso_detectado = "1" # Default
        if "Piso 1" in sl: piso_detectado = "Piso 1"
        elif "Piso 2" in sl: piso_detectado = "Piso 2"
        elif "Piso 3" in sl: piso_detectado = "Piso 3"
        ...
        with st.form("res_s"):
            ...
            if st.form_submit_button("Reservar"):
                if check_room_conflict(get_room_reservations_df(conn).to_dict("records"), str(fe), sl, i, f):
                    st.error("Sala ocupada.")
                else:
                    add_room_reservation(conn, n, e, piso_detectado, sl, str(fe), i, f, datetime.datetime.now().isoformat())
                    st.success(f"Reservado: {sl}")
...
# ==========================================
    ...
    if st.button("Salir"): st.session_state["is_admin"] = False; st.rerun()
    ...
    # -------------------------------------------------------
    # T2: EDITOR VISUAL (Optimizado para carga)
    # -------------------------------------------------------
    with t2:
        ...
        if not pim:
            ...
            # Se mantiene la lógica robusta pero solo se ejecuta si pim existe
            try:
                bg_image = PILImage.open(pim).convert("RGB")
                cw = 800; w_orig, h_orig = bg_image.size
                ch = int(h_orig * (cw / w_orig))
                bg_image_resized = bg_image.resize((cw, ch)) # Redimensionar antes
                
                canvas = st_canvas(
                    fill_color="rgba(0, 160, 74, 0.3)", stroke_width=2, stroke_color="#00A04A",
                    background_image=bg_image_resized, update_streamlit=True,
                    width=cw, height=ch, drawing_mode="rect", key=f"cv_{p_sel}_v3"
                )
    ...
    with t3:
        ...
        # Generación de PDF usando la función corregida
    ...
    with t4:
        n_u = st.text_input("Nuevo User")
        n_p = st.text_input("Nueva Pass", type="password")
        if st.button("Actualizar"):
            save_setting(conn, "admin_user", n_u); save_setting(conn, "admin_pass", n_p); st.success("Listo")

    with t5: admin_appearance_ui(conn)
    
    # ARREGLO ERROR 2: Restauración de opciones de Mantenimiento
    with t6:
