...
# ==========================================
# NUEVAS FUNCIONES DE DISEÑO MODERNO
# ==========================================
...
def modern_zone_designer(p_sel, d_sel, zonas, df_d, global_logo_path):
    ...
    with tab1:
        # Usar Plotly para vista principal
...
def create_plotly_office_viz(p_sel, d_sel, zonas, df_d):
    ...
    st.subheader("🏢 Vista Interactiva del Espacio")
    ...
    if not pim.exists():
        st.warning("No se encontró el plano base para visualización")
        return
    
    # Crear figura de Plotly
    fig = go.Figure()
    
    # Aquí agregarías la lógica para dibujar las zonas
    # Por ahora, mostramos un mensaje y la imagen estática
    st.info("🔄 Visualización interactiva en desarrollo...")
    st.image(str(pim), use_container_width=True, caption=f"Plano base del {p_sel}")

def modern_zone_editor(p_sel, d_sel, zonas, df_d):
    """Editor moderno de zonas"""
    
    st.subheader("✨ Crear Nueva Zona")
    
    with st.expander("🛠️ Herramientas de Diseño", expanded=True):
        col1, col2 = st.columns(2)
        
        with col1:
            # Selector de equipo con estilo
            ...
            equipo = st.selectbox("Equipo/Sala", eqs, label_visibility="collapsed")
        ...
        with col2:
            ...
            color = st.color_picker("Color de la zona", "#00A04A", label_visibility="collapsed")
    ...
    with col_size:
        ...
        col_x, col_y = st.columns(2)
        with col_x:
            x = st.slider("Posición X", 0, 1000, 100, 10, 
                         help="Posición horizontal en el plano")
        with col_y:
            y = st.slider("Posición Y", 0, 1000, 100, 10,
                         help="Posición vertical en el plano")
        
        col_w, col_h = st.columns(2)
        with col_w:
            w = st.slider("Ancho", 50, 500, 150, 10,
                         help="Ancho del área")
        with col_h:
            h = st.slider("Alto", 50, 500, 100, 10,
                         help="Alto del área")
...
def show_zone_analytics(p_sel, zonas, df_d):
    ...
    with col1:
        st.metric("Total Zonas", total_zones)
    with col2:
        st.metric("Área Total", f"{total_area:,} px²")
    with col3:
        st.metric("Equipos Únicos", unique_teams)
    with col4:
...
# ==========================================
    ...
    with t2:
        # AGREGAR ESTA OPCIÓN AL PRINCIPIO
        st.subheader("🔄 Selecciona el Modo de Edición")
        
        modo_edicion = st.radio(
            "Elige el editor:",
            ["🎨 Editor Moderno (Nuevo)", "⚙️ Editor Avanzado (Original)"],
            horizontal=True
        )
        ...
        # LLAMAR AL EDITOR CORRESPONDIENTE
        if modo_edicion == "🎨 Editor Moderno (Nuevo)":
