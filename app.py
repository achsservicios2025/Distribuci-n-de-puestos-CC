...
def clean_reservation_df(df, tipo="puesto"):
    ...
    # Estandarizar nombres para evitar KeyError
    if tipo == "puesto":
        # Mapeo seguro
        rename_map = {
            'user_name': 'Nombre', 
        ...
        # Seleccionar solo lo que existe
...
# ==========================================
    ...
    with t2:
        ...
        if pim.exists():
            # CARGAMOS EL OBJETO IMAGEN, NO UN STRING BASE64
            img = PILImage.open(pim)
            cw = 800; w, h = img.size
            ch = int(h * (cw / w)) if w > cw else h; cw = w if w <= cw else cw

            # --- CORRECCIÓN FINAL: Pasar 'img' (objeto) al canvas ---
            canvas = st_canvas(
                fill_color="rgba(0, 160, 74, 0.3)",
                stroke_width=2,
                stroke_color="#00A04A",
                background_image=img,  # <--- AQUÍ ESTÁ EL CAMBIO IMPORTANTE
