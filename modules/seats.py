...
def compute_distribution_from_excel(equipos_df, parametros_df, df_capacidades, cupos_reserva=2, ignore_params=False):
    ...
    for piso_raw in pisos_unicos:
        # PISO LIMPIO: Esta variable es la clave. La usamos para todo.
        piso_str = extract_clean_number_str(piso_raw)
        
        # Si no podemos determinar el piso (ej: celda vacía), saltamos
        ...
        if piso_str in capacidad_pisos:
            ...
            # Fallback: Suma de personas + reserva si no hay dato en hoja Capacidades
        ...
        for dia_idx, dia in enumerate(dias_semana):
            ...
            for t in teams:
                if t['asig'] > 0:
                    ...
                    # --- CORRECCIÓN CRÍTICA: USAR SIEMPRE piso_str ---
                    rows.append({
                        "piso": piso_str, # Antes: str(piso_raw) -> ERROR
                        "equipo": t['eq'], 
                        "dia": dia, 
                        "cupos": int(t['asig']), 
                        "pct": pct
            ...
            # --- CORRECCIÓN CRÍTICA ---
            rows.append({
                "piso": piso_str, # Antes: str(piso_raw)
                "equipo": "Cupos libres", 
                "dia": dia, 
                "cupos": int(remanente), 
                "pct": pct_lib
